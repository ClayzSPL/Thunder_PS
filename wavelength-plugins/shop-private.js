...
function assembleOutput(output, marquee, shop, selectionType, back, update, user, room, cache) {
	...
	shop = '<div style="padding: 5px; border: 2px solid #101ad1; background: #6600CC; color: #fff; text-shadow: none; border-radius: 6px; margin: 10px"><center><font size=4><i><b>' + serverName + ' Shop</b></i></font></center></div><br/><br/>' + output;
...
function successfulTransaction(item, receipt, user, room) {
	...
	let output = '<div style="padding: 5px; border: 2px solid #000; background: #6600CC; color: #fff; text-shadow: none; border-radius: 6px; margin: 10px; text-align: center"><font size=4><i><b>' + serverName + ' Shop</b></i></font></div><br/><br/>';
	output += '<center><img src="pokemonshowdown.com/sprites/xyani/marshadow><b>Your purchase was successful</b><br/><br/>' + item.onbuy + '<br/></center><div style="width: 70%;margin: 0 auto;"><b>Name: </b>' + user.name + '<br/><b>Date: </b>' + receipt[1] + '<br/><b>Item: </b>' + receipt[2] + '<br/><b>Price: </b>' + receipt[3] + ' ' + receipt[4] + '<br/><b>Remaining ' + receipt[4] + ': </b>' + receipt[6] + '<br/><b>ReceiptID: </b>' + receipt[5] + '<br/><br/><center>This receipt is stored as your proof of purchase in the event of an error.<br/>Use /receipts to see your recent receipts.<img src="pokemonshowdown.com/sprites/xyani/marshadow><br/><br/>';
	...
	output += '<br/></center></div><div style="padding: 5px; border: 2px solid #101ad1; background: #6600cc; color: #fff; text-shadow: none; border-radius: 6px; margin: 10px"><div style="float: left;"><button style="border: 2px solid #000; border-radius: 6px; background: #6600CC; color: white;" name="send" value="/shop main|1">Main Menu</button> <button style="border: 2px solid #000; border-radius: 6px; background: #6600CC; color: white;" name="send" value="/shop exit|1">Exit</button></div><marquee style="text-align: center; width: 70%" direction="left"><img src="pokemonshowdown.com/sprites/xyani/marshadow>Thank you for your purchase!<img src="pokemonshowdown.com/sprites/xyani/marshadow></marquee></div>';
...
exports.commands = {
	...
	shop: function (target, room, user) {
		...
		let selection = false;
		let isUpdate;
		let marquee = '<img src="pokemonshowdown.com/sprites/xyani/marshadow> Welcome to the ' + serverName + ' Shop! Credit for the original code goes to Lights; any feedback is welcome. <img src="pokemonshowdown.com/sprites/xyani/marshadow>';
		if (target) {
			if (target.trim() === 'help') {
