...
class PassTheBomb {
	...
	getMsg() {
		...
		msg += `Players: ${this.getSurvivors().map(player => WL.nameColor(player[1].name).join(", "))}<br />`;
	...
	nextRound() {
		...
		this.release = setTimeout(() => {
			...
			this.room.add(`|uhtmlchange|${this.getMsg()}<br /><strong style="font-size: 10pt;">The bomb has been passed to </strong>${WL.nameColor(player, true)}</div>`).update();
			this.canPass = true;
			this.resetTimer();
	...
	pass(user, target, self) {
		...
		this.room.add(`|html|${WL.nameColor(user.name, true)} passed the bomb to ${WL.nameColor(targetId, true)}</strong>!`);
	...
	resetTimer() {
		this.timer = setTimeout(() => {
			let player = this.players.get(this.holder).name;
			this.room.add(`|html|The bomb exploded and killed ${WL.nameColor(player, true)}`).update();
	...
	getWinner() {
		...
		let msg = `|html|<div class="infobox"><center>The winner of this game of Pass The Bomb is ${WL.nameColor(winner, true)}! Congratulations!</center>`;
		if (this.room.isOfficial) {
			WL.ExpControl.addExp(winner, this.room, 5);
			msg += `${WL.nameColor(winner, true)} has also won 5 EXP for winning this game of Pass The Bomb.`;
		}
...
exports.commands = {
	...
	passthebomb: {
		...
		proceed: function (target, room, user) {
			...
			room.addRaw(`(${WL.nameColor(user.name, true, true)} forcibly started the game of Pass The Bomb.)`);
