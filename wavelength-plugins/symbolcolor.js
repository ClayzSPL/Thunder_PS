...
exports.commands = {
	...
	sc: {
		...
		set: function (target, room, user) {
			...
			this.sendReply(`|raw|You have given ${WL.nameColor(target[0], true)} an symbol color.`);
			Monitor.log(`${target[0]} has received an symbol color from ${user.name}.`);
			this.privateModAction(`|raw|(${WL.nameColor(target[0], true)} has received a symbol color: <font color="${target[1]}">${target[1]}</font> from ${user.name}.)`);
