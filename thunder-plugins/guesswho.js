...
class GuessWho {
	...
	guess(user, guess) {
		...
		if (guess.species === this.answer.species) {
			this.room.add(`|html|${WL.nameColor(user.name, true)} guessed <strong>${guess.species}</strong>, which was the correct answer! ${WL.nameColor(user.name, true)} has also won 5 ${moneyPlural}! ${WL.nameColor(user.name, true)} has also won 5 EXP!`);
			...
			WL.ExpControl.addExp(user.userid, this.room, 5);
			this.end();
		} else {
			this.guessed[toId(guess.species)] = user.userid;
			this.guessedPokemon.push(guess.species);
			this.guesses--;
			this.room.add(`|html|${WL.nameColor(user.name, true)} guessed <strong>${guess.species}</strong>, but that was not the correct answer. <strong>${this.guesses} guesses are left.</strong>`);
			if (this.guesses < 1) {
				this.room.add(`|html|Sorry, the guessers have lost this round. Congratulations to the ${WL.nameColor(this.questionee, true)}! Their Pokemon was ${this.answer.species}! ${WL.nameColor(this.questionee, true)} has won ${this.prize} ${moneyPlural}! ${WL.nameColor(this.questionee, true)} has also won 5 EXP!`);
				...
				WL.ExpControl.addExp(this.questionee, this.room, 5);
				this.end();
			}
			if (this.guesses === 7 && this.hints.length < 2) {
				this.room.add(`|html|${WL.nameColor(this.questionee, true)}, it is now time for you to give the guessers their 2nd hint.`);
			}
			if (this.guesses === 3 && this.hints.length < 3) {
				this.room.add(`|html|${WL.nameColor(this.questionee, true)}, it is now time for you to give the guessers their 3rd hint.`);
			}
		}
	}
	...
	start() {
		...
		this.room.add(`|uhtmlchange|guesswho-${this.gwNumber}|<div class="infobox">This Guess Who game has been started! ${WL.nameColor(this.questionee, true)} is the questionee! Begin guessing ${Chat.toListString(this.players)}!`);
		this.room.add(`|html|${WL.nameColor(this.questionee, true)}, check <button name = "send" value = "/guesswho showanswer">here</button> to view the Pok&eacute;mon and begin giving hints.`);
	}
	...
	giveHint(hint) {
		this.room.add(`|html|${WL.nameColor(this.questionee, true)} has given the hint: <strong>"${hint}"</strong>.`);
